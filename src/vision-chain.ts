...
export async function analyzeScreenshotChain(
  ...
  // ⚠️ 只给经纬度、**不给像素坐标**：像素↔经纬度换算已停止开放，报像素只会引模型去谈像素精度，
  // 而实测视觉模型不具备像素测量能力（16% 图宽 ≈ 234m 误差）。视觉这边只做「识别语义」。
  const where = shot.pinned
    ? '图中红色圆点是用户点击的关注位置，'
    : '这张图是当前视野的捕获，**图上没有任何标记**（不要假设有红点或图钉）；画面中心'
  const prompt = `这是用户地图的截图（截图上无文字坐标）。${where}`
