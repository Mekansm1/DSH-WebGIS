...
import { registerPixelTools } from './pixel-tools.js'
...
/**
 * 是否把像素↔经纬度换算工具（`webgis_project` / `webgis_unproject`）开放给模型。
 *
 * **默认 false = 停止开放**（2026-09-13 用户决定）。代码保留在 `pixel-tools.ts`，
 * 换算函数仍被截图地理范围计算复用；这里只管「要不要注册成工具」。
 * 关闭后必须同时保证**没有任何提示词再叫模型去调这两个工具**，否则模型会对着不存在的
 * 工具名产生幻觉（见 ARCHITECTURE §6「工具描述必须与代码一致」）。
 * 要重新开放改成 true 即可。
 */
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      '【导出地图为图片】把当前地图导出成一张带版式的图片（可带标题/图例/指北针/比例尺）。'
      + '⚠ 三个「导出」别混：本工具产**图片**；用户要**数据文件**（CSV/GeoJSON）用 webgis_export_layer；'
    ...
    output: {
      ...
      render: (_args, value) => {
        ...
        const layersText = items.length > 0
          ...
          : '当前画面上没有可见图层。'
        // 像素↔经纬度换算工具已停止开放（见 PIXEL_TOOLS_ENABLED）：这里**绝不能**再提示模型去调
        // webgis_project / webgis_unproject —— 那会让它对着不存在的工具名产生幻觉。
        const body = `${head}\n${layersText}`
        if (!v.attachImage) {
  ...
  // ---- 像素↔经纬度换算工具：已停止开放（代码保留在 pixel-tools.ts）----
  // 关掉的原因见该文件头部：实测视觉模型不具备像素坐标测量能力（16% 图宽 ≈ 234m 误差），
  // 而程序化像素扫描 ≈11m —— 让模型估像素再换算这条路本身不成立。
  // 保留代码（geo.ts 的换算函数另被截图地理范围计算复用），只是不注册、模型看不到。
  if (PIXEL_TOOLS_ENABLED) registerPixelTools(ctx, stateFor)
